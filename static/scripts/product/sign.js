!function () {
    ...
    play.getVideo = setInterval(function () {
        ...
        if (play.media&&play.media.readyState > 0) {
    ...
    play.elmeEvent = function () {
        ...
        $(play.addBtn).bind('click', function (e) {
            ...
            if (!navigator.cookieEnabled) {
                ...
                if($("#videoId").val()==3){
                    ...
                    $('.desc').val("");
                    for(var i= 1;i<=10;i++){
                        if($(".questionBtn."+i))$(".questionBtn."+i).remove();
                    }
                    $("#questionDetail").val("");
                    $("#questionDetail").hide();
        ...
        $('body').delegate('.questionAdd', {
            click: function () {
                if($(".questionBtn").length<10){
                    $(this).before("<a class=\"btn questionBtn "+(1+$(".questionBtn").length)+"\" style=\"margin:2px\" data-type='1'>"+(1+$(".questionBtn").length)+"</a>")
                    $(this).prev("a").click();
        ...
        //see question
        $('body').delegate('.questionBtn', {
            click: function () {
                $('.questionBtn').removeClass("inedit");
                $(this).addClass("inedit");
                $('#questionDetail').val($(this).val());
                $('#questionDetail').fadeIn(500);
                $('input[name="questype"][value="'+($(this).attr("data-type")||0)+'"]')[0].checked=true;
                $('#questionTypeGroup').fadeIn(500);
            }
        });
        //see question
        $('body').delegate('#questionDetail', {
            change: function () {
                $('.questionBtn.inedit').val($(this).val());
                console.log($('.questionBtn.inedit').val())
            }
        });
        $('body').delegate('input[name="questype"]', {
            click: function () {
                $('.questionBtn.inedit').attr("data-type",$(this).val());
                console.log($('.questionBtn.inedit').attr("data-type"))
            }
        });
        //选择某个素材
...
//提交按钮
        $('body').delegate('#submit2', {
            ...
            click: function () {
                ...
                ;
                var question=[]
                for(var i= 1;i<=$(".questionBtn").length;i++){
                    question.push({text:$(".questionBtn."+i).val()||"",type:$(".questionBtn."+i).attr("data-type")});
                    //question[i]=$(".questionBtn."+i)?$(".questionBtn."+i).val():"";
                }
        ...
        if (play.ads) {
            ...
            $.each(play.ads, function (key, val) {
                ...
                if (_time == val['time']) {
                    ...
                    if (val['desc'] == '' && val['img'] == null &&val['url']!=null) {
                        //if(val['img'] == null){
